...
while True:
  ...
  mse_train=mean_squared_error(y_train, lassoregr.predict(x_train))
  mse_test=mean_squared_error(y_test, lassoregr.predict(x_test))
  score_test=r2_score(y_test,lassoregr.predict(x_test))
  score_train=r2_score(y_train,lassoregr.predict(x_train))
  if p<0.05 and score_test>0 and score_cv>0:
    break
  i=i+1
print('R2 CV (LASSO): ',100 * score_cv," %")
print('RMSE CV (LASSO): ',np.sqrt(mse_cv))
...
#print("Best alpha : ", lassoregr.alpha_)
s=0
cumperc=[]
perc_coefs=DataFrame({'idx':results['PC'].columns,'val':[abs(i)/np.sum(abs(lassoregr.coef_)) for i in lassoregr.coef_],'cum':range(len(results['PC'].columns))}).sort_values(by=['val'])
